...
参考文献:
    ...
    def __init__(self, problem, population):
        ...
        if population.Encoding == 'P':
            ...
            self.mutDE = DE_rand_1() # 生成差分变异算子对象
    ...
    def run(self, prophetPop = None): # prophetPop为先知种群（即包含先验知识的种群）
        ...
        self.neighborSize = NIND//10
        self.neighborSize = max(self.neighborSize, 2) # 确保不小于2
        self.nr = NIND//100  #max number of solutions replaced by each offspring
        # 生成由所有邻居索引组成的矩阵
        ...
        while self.terminated(population) == False:
            ...
            for i in range(population.sizes):
                ...
                if select_rands[i] < self.Ps:
                    ...
                    chooseIdx = neighborIdx[i]
                else:
                    # chooseIdx = ea.rps(population.sizes, 2)
                    chooseIdx = np.arange(NIND)
                ...
                offspring.Chrom = self.mutDE.do(population.Chrom, population.Field, i, chooseIdx)
                ...
                # 重插入更新种群个体
                self.reinsertion(chooseIdx, population, offspring, idealPoint, uniformPoint)

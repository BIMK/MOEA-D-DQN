# -*- coding: utf-8 -*-
import logging
from moea_MOEAD_DRA_DE_templet import moea_MOEAD_DRA_DE_templet
from moea_MOEAD_DE_templet import moea_MOEAD_DE_templet
...
logging.basicConfig(
    handlers=[logging.FileHandler(
        "./result/RL_UF1-7.log", encoding="utf-8", mode='w')],
    level=logging.INFO,
...
if __name__ == '__main__':
    ...
    for problemName in problems:
        ...
        MyProblem = getattr(__import__(
            'problem.'+problemName), problemName)  # 导入自定义问题类
        MyProblem = getattr(MyProblem, problemName)
        problem = MyProblem()       # 生成问题对象--DTL设置为3目标
        ...
        for i in range(N):
            ...
            IGD1 = ea.indicator.IGD(NDSet.ObjV, PF)     # 计算IGD指标
            HV = ea.indicator.HV(NDSet.ObjV, PF)       # 计算HV指标
            logging.info('round %d/%d, IGD = %.7f, HV = %.7f'%(i+1,N,IGD,HV))
            # print(IGD1)
            ...
            plt.show()
            plt.close()
        logging.info('median ----- IDG = %.7f, IGD.std = %.7f, HV = %.7f, HV.std = %.7f' % (np.median(igd), np.std(igd), np.median(hv), np.std(hv) ))

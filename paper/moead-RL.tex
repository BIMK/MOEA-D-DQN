  ...
  \item Simulated binary crossover
        \begin{equation}
          \beta=\left\{\begin{array}{cl}
            (2 u)^{\frac{1}{n_{c+1}}},                         & \text { if } u \leq 0.5 \\
            \left(\frac{1}{2(1-u)}\right)^{\frac{1}{n_{c+1}}}, & \text { otherwise }
          \end{array}\right.
        \end{equation}
...
\begin{algorithm}[t]
  ...
  $Q$: The primary action-value network\;
  $Q^\prime$: The target action-value network\;
  ...
  Define $a_j^{max} = \argmax_{a^\prime} Q(s_{j+1}, a)$\;
  $y_j = r + \gamma Q^\prime(s_{j+1}, a_j^{max})$\;
  ...
  Every $C$ steps, copy the weight of $Q$ to $Q^\prime$\;
  }
  % 更新pi
  $gen$++\;
  \If {$mod(gen,50)=0$}{

...
\begin{abstract}
  ...
  % 本文提出了一种基于DDQN的算子选择策略，来确定哪个算子将被应用
...
% 本文组织结构
% 在第二章，我们介绍了多目标优化，强化学习的和AOS的相关工作. 在第三章，我们提出了MOEA/D-DDQN，并详细介绍了实现细节。实验结果
...
\begin{algorithm}[t]
  ...
  \small
  \KwIn{\textit{op} (applied operator), $x$ (generated offspring)}   % 输入子代和应用的算子
  \KwOut{\textit{reward}}   % 
  % 初始化部分变量
  $Q \leftarrow $ Queue store \textit{NFI} values and applied operators\;  %队列
  ...
  $\textit{reward} = 0$\;
  \For {$i \leftarrow 1 $\textbf{ to }$ Q.\textit{length}$}{
    \If {$Q_i.\textit{op} == op$}{
      $\textit{reward} = \max\{\textit{reward}, Q_i.\textit{op}\}$\;
    }
  }
  \Return{\textit{reward}}\;
\end{algorithm}
...
\begin{algorithm}
  ...
  \KwOut{\textit{op}}   %  输出选择的算子
  % 统计Q中每个算子应用的次数，如果有算子的应用次数是零，直接返回这个算子
  \For {$i \leftarrow 1 \textbf{ to } W$}{
    $\textit{op} = Q_i.\textit{op}$\;
    $n_{op}$++\;
  }
...
\begin{algorithm}[t]
  \label{alg:moead-ddqn}
  \caption{Framework of MOEA/D-DDQN}
  \SetKwComment{Comment}{//}{}
  \small
  % 初始化各种参数
  % while 评价次数t<FE^max
  % 索引 i in (1,,NP)
  % 组成state s_t = {x_i, w_i}
  % 根据状态选择算子,此处参考 operator selection
  % a_t = OS(s_t)
  % generate trial vector u_i for parent x_i using operator a_t
  % 计算reward，此处参考 credit assignment
  % 更新邻域中的solutions 
...
% 在时刻t，个体的决策变量和权重向量共同组成state s，作为神经网络的输入，神经网络的输出是一个n维向量 out，表示应用算子后可以得到的期望累计回报，根据输出向量基于轮盘赌选择一个算子op，应用算子产生子代off，在基于分解的多目标框架中，我们可以很容易的计算子代相对于父代的适应度值提高率FIR，将应用的算子和得到的FIR放入滑动窗口中，之后从滑动窗口中取出算子op对应的最大FIR作为reward r，使用DDQN算法更新RL的策略以最大化累积奖励。
...
%%% 实验研究
% 为了证明算法MOEA/D-DDQN的有效性，我们在 xxx 数据集进行了测试，并且和MOEA/D-DE,MOEA/D-DRA,MOEA/D-FRRMAB,MOEA/D-IR比较了结果。
...
% 我们分别使用两个神经网络推荐交叉算子和变异算子，除了输入层的维度，两个网络具有相同的结构。
...
% 使用dark和light gray指示最佳和次优的结果 
% Dark/light gray emphasizes the best/second-best results.

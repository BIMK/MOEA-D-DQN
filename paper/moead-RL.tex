...
% 强化学习介绍
Reinforcement Learning (RL) \cite{sutton2018reinforcement} is an area of machine learning concerned with how intelligent agents ought to take actions in an environment, and the goal of agents is to maximize the notion of cumulative reward \cite{van2012reinforcement}. Unlike supervised learning, RL learns by trial-and-error and does not require training samples. Therefore, RL pays more attention to find a balance between exploration (of uncharted territory) and exploitation (of current knowledge) \cite{kaelbling1996reinforcement}.

% 基本的强化学习过程
The typical model of RL can be described as: an agent takes actions according to the state given by environment, the environment accepts the action and returns the reward to the agent, and the goad of RL is to adjust the strategy to maximize the reward.

% 由于强化学习具有平衡探索和利用的特性，它可以被用来解决D-mab问题
Since RL has the feature of balance EvE, it can be used to solve dynamic MAB problem.

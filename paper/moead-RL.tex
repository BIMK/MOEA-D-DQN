...
So in order to avoid the operator probability is too small to be selected, PM designs a minimal probability, $p_{min}: 1 > p_{min} > 0$, for exploring currently underperforming operator. Since the total probability is 1, then the maximum probability of an operator being selected is $p_{max} = 1-(k-1) P_{min}$ where $k$ is the number of operators, thus we can get the update rule of $p_{i,t}$:
$$
...
% AP的过程介绍
Like PM, AP also designs the minimum probability $p_{min}$

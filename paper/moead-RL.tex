...
\begin{abstract}
  ...
  % 在进化过程中，
  In the evolutionary process, RL will select an operator for each individual and update the behavioral strategy according to the effect of this selection. Experimental results on \TODO{supplement experimental instructions}.
\end{abstract}
...
% RC的设计参考了模拟退火的思想，随着进化代数的增加逐渐趋近0
% The design of $RC$ is based on the idea 
...
%%%% 为了使得每个算子都能得到应用，队列中每个算子至少有一条记录，也即是说，如果算子没有在队列中，那么下次一定会应用哪个算子。
% 为了保证reward_k存在，队列中每个算子至少被应用一次，也就是说，如果唯一的A(op_i,fir)被移出队列，那么下一次一定会选择算子i

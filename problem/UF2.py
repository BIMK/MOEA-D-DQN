...
class UF2(ea.Problem): # 继承Problem父类
    ...
    def aimFunc(self, pop): # 目标函数
        ...
        yJ1 = Vars[:, J1] - (0.3 * x1**2 * np.cos(24 * np.pi * x1 + 4 * (J1 + 1) * np.pi / self.Dim) + 0.6 * x1) * np.cos(6 * np.pi * x1 + (J1 + 1) * np.pi / self.Dim)
        yJ2 = Vars[:, J2] - (0.3 * x1**2 * np.cos(24 * np.pi * x1 + 4 * (J2 + 1) * np.pi / self.Dim) + 0.6 * x1) * np.sin(6 * np.pi * x1 + (J2 + 1) * np.pi / self.Dim)

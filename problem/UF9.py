...
class UF9(ea.Problem): # 继承Problem父类
    def __init__(self):
        name      = 'UF9' # 初始化name（函数名称，可以随意设置）
        M         = 3  # 初始化M（目标维数）
    ...
    def aimFunc(self, pop):  # 目标函数
        ...
        f1   = 0.5*(np.maximum(0,(1.1*(1-4*(2*x1-1)**2)))+2*x1)*x2   + 2*np.mean(f[:, J1], 1, keepdims = True)
        f2   = 0.5*(np.maximum(0,(1.1*(1-4*(2*x1-1)**2)))-2*x1+2)*x2 + 2*np.mean(f[:, J2], 1, keepdims = True)
